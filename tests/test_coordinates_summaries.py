"""Regression coverage for review item 6: the combined two-build report must
carry every prespecified scientific table — mapping rates, the
...
sensitivity, control alerts, and retention analyses by protein/class, GC, and
low complexity. These are schema + basic-correctness assertions on
...
PRIMARY_ROWS = [
    ...
    _row("row_3", "primary", "unmapped", chrom="", start="", end="", strand=""),
...
SPLICE_ROWS = [
    ...
    _row("row_3", "splice", "unmapped", chrom="", start="", end="", strand=""),
...
class BuildPerBuildSummarySchemaTests(unittest.TestCase):
    ...
    def test_strand_and_locus_table(self):
        ...
        # row_0, row_1, row_2, control_row_0 mapped (chrom set); row_3
        # unmapped (excluded). Strand/locus summaries include controls.
        self.assertEqual(payload["primary"]["distinct_loci"], 4)

    def test_near_tied_sensitivity_table_covers_configured_fractions(self):
        ...
        # row_2's primary row declared near_tied_fractions "0.05;0.1".
    ...
    def test_retention_table_has_protein_class_gc_and_low_complexity(self):
        retention = self.summary["retention"]
        for key in ("by_protein_class", "by_gc_decile", "by_low_complexity_decile"):
            self.assertIn(key, retention)

        by_protein_class = retention["by_protein_class"]
        for key in ("overall", "overall_positive_negative_gap", "per_protein_class", "per_protein_gap_distribution", "severe_alerts"):
            self.assertIn(key, by_protein_class)
        # protein 1: row_0 positive usable(True), row_1 negative usable(True,
        # rescued by splice) -> both classes retain 100%, no severe alert.
        self.assertIn("1:positive", by_protein_class["per_protein_class"])
        self.assertIn("1:negative", by_protein_class["per_protein_class"])

        gc_bucket = gc_decile(SAMPLE_SEQUENCES["row_0"])
        self.assertIn(str(gc_bucket), retention["by_gc_decile"])
        complexity_bucket = low_complexity_decile(SAMPLE_SEQUENCES["row_1"])
        self.assertIn(str(complexity_bucket), retention["by_low_complexity_decile"])
    ...
    def test_severe_alert_triggers_on_40_point_positive_negative_gap(self):
        primary_rows = [
            _row("pos_1", "primary", "exact_unique"),
            _row("neg_1", "primary", "unmapped", chrom="", start="", end="", strand=""),
        ]
        splice_rows = [
            _row("pos_1", "splice", "unspliced_unique"),
        ...
        self.assertTrue(
            any(alert["reason"] == "retention_gap_ge_40pts" and alert["protein"] == 9 for alert in retention["severe_alerts"])
        )


class CompareBuildsSchemaTests(unittest.TestCase):
    def test_compare_builds_reports_required_fields(self):
        ...
        # hg19 differs only in row_0's locus (a different chrom): a locus
        # change should be detected.
        primary_rows_hg19 = [
            _row("row_0", "primary", "exact_unique", chrom="chr9", start="0", end="20"),
            *PRIMARY_ROWS[1:],
        ]
        ...
        comparison = summaries.compare_builds(
            ...
            {
                "hg38": summaries.index_rows_by_sample(PRIMARY_ROWS, mode="primary"),
                "hg19": summaries.index_rows_by_sample(primary_rows_hg19, mode="primary"),
            },
        )
        for key in (
            "builds_compared",
            ...
            "loci_changed_between_builds",
            "loci_compared",
        ):
            self.assertIn(key, comparison)
        self.assertEqual(comparison["loci_changed_between_builds"], 1)
