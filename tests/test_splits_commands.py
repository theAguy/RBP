...
class CommandBuilderTests(unittest.TestCase):
    ...
    def test_cluster_command_matches_frozen_flags_exactly(self):
        cmd = cluster_command(Path("db"), Path("clu"), Path("tmp"))
        self.assertEqual(
            list(cmd.argv),
            [
    ...
    def test_audit_search_command_pins_search_type_and_strand(self):
        cmd = audit_search_command(Path("q"), Path("t"), Path("res"), Path("tmp"))
        argv = list(cmd.argv)
        ...
        # Same shared alignment/coverage/e-value/mask/sensitivity flags as cluster.
        for flag, value in (("--alignment-mode", "3"), ("--cov-mode", "0"), ("-e", "1000"), ("--mask", "0"), ("-s", "7.5")):
            self.assertEqual(argv[argv.index(flag) + 1], value)

    def test_audit_search_command_and_cluster_command_are_distinct_workflows(self):
        cluster_argv = list(cluster_command(Path("db"), Path("clu"), Path("tmp")).argv)
        search_argv = list(audit_search_command(Path("q"), Path("t"), Path("res"), Path("tmp")).argv)
...
class RunMmseqsCommandTests(unittest.TestCase):
    ...
    def test_non_zero_exit_raises_and_preserves_stderr_log(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            stderr_log = log_dir / "mmseqs_cluster.stderr.log"
            self.assertTrue(stderr_log.is_file())
            self.assertIn("fatal: bad flag", stderr_log.read_text())
